...
if inGame() and isPlaying() and isHost() and not inChat() then
	...
	managers.hud:show_hint( { text = "SWAT 2" } )
	...
	managers.hud:show_hint( { text = "SNIPER 2" } )
	...
	elseif data_sb then
		spawn_bag('diamonds')
	if is_french then
	managers.hud:show_hint( { text = "BIJOUX" } )
	else
	managers.hud:show_hint( { text = "DIAMONDS" } )
	end
	elseif data_sbs then
		spawn_bag('person')
	managers.hud:show_hint( { text = "BODYBAG" } )
	elseif data_sr then
		rain_bag('diamonds')
	if is_french then
	managers.hud:show_hint( { text = "BIJOUX" } )
